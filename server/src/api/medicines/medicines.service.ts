...
export class MedicinesService {
    ...
    async findOne(auth: Auth, id: string) {
        ...
        if (auth?.allows.some((allows) => allows.edit.some((allow) => allow === Allow.MEDICINES)))
            return medicine;
        return this.apiService.findOne(auth, medicine);
    }

    async create(auth: Auth, data: CreateMedicineDto) {
        const newData = await this.apiService.create<Medicine, CreateMedicineDto>(
            auth,
            data,
            (data) => ({
                name: data.name,
    ...
    async createMany(auth: Auth, data: CreateMedicineDto[]) {
        const newData = await this.apiService.createMany<Medicine, CreateMedicineDto>(
            auth,
            data,
            (data) => ({
                name: data.name,
    ...
    async update(auth: Auth, id: string, data: UpdateMedicineDto) {
        const medicine = await this.medicineModel.findById(id);
        const newData = await this.apiService.update<Medicine, MedicineDocument, UpdateMedicineDto>(
            auth,
            medicine,
            data,
            (data) => ({
